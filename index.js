...
// Business knowledge starts from env vars (set once at deploy time) but can be
// updated live from the app's AI Training page via POST /api/business-knowledge --
// this is what lets WhatsApp auto-replies actually know about the real business,
// instead of being stuck with whatever was configured on day one.
let bizKnowledge = {
  name: process.env.BIZ_NAME || "the business",
  description: process.env.BIZ_DESCRIPTION || "",
  pricing: process.env.BIZ_PRICING || "",
  faqs: process.env.BIZ_FAQS || "",
  script: process.env.BIZ_SCRIPT || "",
};
...
async function callGemini(system, messages) {
  ...
  const response = await ai.models.generateContent({
    model: "gemini-3.6-flash", // current stable model as of July 2026 (2.5-flash was blocked for new accounts)
    contents: contents,
    config: {
      systemInstruction: system,
      tools: [{ googleSearch: {} }], // lets the model search live when a question needs current info
    },
  });
...
async function triggerAutoReply(jid) {
  ...
  const system =
    `You are auto-replying on WhatsApp on behalf of ${bizKnowledge.name}, stepping in only because ` +
    `no human on the team replied within ${AUTOREPLY_MINUTES} minute${AUTOREPLY_MINUTES === 1 ? "" : "s"}. ` +
    (bizKnowledge.description ? `Business: ${bizKnowledge.description}. ` : "") +
    (bizKnowledge.pricing ? `Pricing: ${bizKnowledge.pricing}. ` : "") +
    (bizKnowledge.faqs ? `Common FAQs: ${bizKnowledge.faqs}. ` : "") +
    (bizKnowledge.script ? `Preferred sales approach: ${bizKnowledge.script}. ` : "") +
...
app.post("/api/ai-chat", async (req, res) => {
  try {
    if (!GEMINI_API_KEY && !ANTHROPIC_API_KEY) {
      console.error("AI-CHAT ERROR: no provider configured");
    ...
    console.log("AI-CHAT request received, provider:", GEMINI_API_KEY ? "gemini" : "anthropic", "messages:", (messages||[]).length);
...
// Called from the app's AI Training page whenever it's saved, so WhatsApp
// auto-replies actually know the real business -- not just whatever was set
// once in Render's environment variables at deploy time.
app.post("/api/business-knowledge", (req, res) => {
  const { name, description, pricing, faqs, script } = req.body || {};
  if (name !== undefined) bizKnowledge.name = name;
  if (description !== undefined) bizKnowledge.description = description;
  if (pricing !== undefined) bizKnowledge.pricing = pricing;
  if (faqs !== undefined) bizKnowledge.faqs = faqs;
  if (script !== undefined) bizKnowledge.script = script;
  console.log("Business knowledge updated:", JSON.stringify(bizKnowledge));
  res.json({ ok: true, bizKnowledge });
});

app.get("/api/business-knowledge", (req, res) => {
  res.json(bizKnowledge);
});
